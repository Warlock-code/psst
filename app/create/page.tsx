...
if (!profile.isPrime && currentStorage + uploadSize > storageLimit) {
  setError("Free ghosts only get 50MB storage.")
  setLoading(false)
  return
}
      const now = new Date()
  ...
  return (
    <main className="min-h-screen px-5 pb-28 pt-8 text-white">
      <section className="mx-auto max-w-md">
        ...
        <p className="mt-3 text-white/60">Text, meme, gossip or poll.</p>
...
{category === "voice" && (
  ...
  (category === "poll" && pollOptions.filter((o) => o.trim() !== "").length < 2)
}
      ...
      <nav className="fixed bottom-4 left-1/2 flex w-[92%] max-w-md -translate-x-1/2 items-center justify-between rounded-3xl border border-white/10 bg-black/80 p-2 backdrop-blur">
        <Link href="/feed" className="rounded-2xl px-4 py-3 text-sm font-bold text-white/70">Feed</Link>
        <Link href="/leaderboard" className="rounded-2xl px-4 py-3 text-sm font-bold text-white/70">Board</Link>
        <Link href="/create" className="rounded-2xl bg-white px-4 py-3 text-sm font-bold text-black">Post</Link>
        <Link
  href="/battles"
...
>
  Battle
        ...
        <Link href="/lair" className="rounded-2xl px-4 py-3 text-sm font-bold text-white/70">Lair</Link>
      </nav>
    </main>
  )
}